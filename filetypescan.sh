...
zenity --info --text="Current available options: .exe, .php, and .zip files. NOTE: .php will scan every php file collected from all websites browsed so use at your own risk" &> /dev/null
selection=$( zenity --forms --title "Select files to scan" --text "Filetype Selection" --add-combo "Choose a filetype to scan below" --combo-values ".exe|.php|.zip|ALL" 2> /dev/null )
